...
static void transform(crypto_sha256_ctx_t *ctx, const uint8_t *block) {
...
void crypto_sha256_init(crypto_sha256_ctx_t *ctx) {
    ...
    ctx->bit_count = 0; ctx->buffer_offset = 0;
}
...
void crypto_sha256_update(crypto_sha256_ctx_t *ctx, const uint8_t *data, size_t len) {
    for (size_t i = 0; i < len; i++) {
        ...
        ctx->bit_count += 8;
        if (ctx->buffer_offset == CRYPTO_SHA256_BLOCK_SIZE) {
...
void crypto_sha256_final(crypto_sha256_ctx_t *ctx, uint8_t *digest) {
    ...
    if (ctx->buffer_offset > 56) {
        while (ctx->buffer_offset < CRYPTO_SHA256_BLOCK_SIZE)
...
int crypto_hmac_sha256(const uint8_t *key, size_t key_len,
                       ...
                       uint8_t *out_mac)
{
    uint8_t k[CRYPTO_SHA256_BLOCK_SIZE];
    uint8_t ipad[CRYPTO_SHA256_BLOCK_SIZE];
    uint8_t opad[CRYPTO_SHA256_BLOCK_SIZE];
    crypto_sha256_ctx_t ctx;

    memset(k, 0, CRYPTO_SHA256_BLOCK_SIZE);
    ...
    crypto_sha256_init(&ctx);
    crypto_sha256_update(&ctx, ipad, CRYPTO_SHA256_BLOCK_SIZE);
    crypto_sha256_update(&ctx, data, data_len);
    crypto_sha256_final(&ctx, out_mac);

    crypto_sha256_init(&ctx);
    crypto_sha256_update(&ctx, opad, CRYPTO_SHA256_BLOCK_SIZE);
    crypto_sha256_update(&ctx, out_mac, CRYPTO_SHA256_DIGEST_SIZE);
    crypto_sha256_final(&ctx, out_mac);
...
{
    (void)modulus; (void)mod_len; (void)exponent;
    (void)digest; (void)digest_size; (void)signature;
    return -1;
}
